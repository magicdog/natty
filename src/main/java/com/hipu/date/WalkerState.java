...
/**
 * used for datewalker.g, supplied method to calculate the date value.
 */
public class WalkerState {
  ...
  private static final Logger _logger = Logger.getLogger("com.hipu.date");
...
//shelton
  ...
  private String span = "";
  private Date _dafaultDate = null;
  ...
  public void seekBySpan(String direction, String seekAmount, String span) {
    ...
    assert(span.equals(DAY) || span.equals(WEEK) || span.equals(MONTH) || 
        ...
        span.equals(SECOND) || span.equals(NAMED_DAY));
    
    boolean isDateSeek = span.equals(DAY) || span.equals(WEEK) || 
      span.equals(MONTH) || span.equals(YEAR) || span.equals(NAMED_DAY);
    ...
    int field = 
      ...
      span.equals(NAMED_DAY) ? Calendar.DAY_OF_YEAR :
  ...
  //we choose the nearest date from now, if the year between 00-99
  		if (year != null) {
  			int newMonth = Integer.parseInt(year);
  			int newYear = getFullYear(dayOfMonthInt);
  			if (newMonth >0 && newMonth<=12) {
  				Calendar cal = Calendar.getInstance();
  				cal.set(Calendar.YEAR, newYear);
  				cal.set(Calendar.MONTH, newMonth);
  				cal.set(Calendar.DAY_OF_MONTH, monthInt);
  				
  				if (Math.abs(System.currentTimeMillis()-cal.getTimeInMillis())
  						< Math.abs(System.currentTimeMillis() - _calendar.getTime().getTime())) {
  					_calendar.set(Calendar.MONTH, newMonth-1);
  					_calendar.set(Calendar.DAY_OF_MONTH, monthInt);
  					_calendar.set(Calendar.YEAR, newYear);
  				}
  			}
  		}
  }
  ...
  private void resetCalendar() {
    ...
    if (_dafaultDate != null) {
    	_calendar.setTime(_dafaultDate);
    }
